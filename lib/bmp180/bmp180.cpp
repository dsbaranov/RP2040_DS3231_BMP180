...
namespace BMP180
{
    ...
    BMP180::BMP180(i2c_inst_t *i2c) : I2CDevice(i2c, REGISTERS::DEVICE_ADDRESS, 22u)
    {
    }
    ...
    bool BMP180::Ping()
    {
        ...
        return data_buffer_[0] == domain::PING_IDENTIFIER;
    }
    void BMP180::readCoefficients()
    {
    ...
    void BMP180::ReadData(bool including_pressure)
    {
        read_temperature();
        read_pressure();
        if (temperature_raw != 0 && pressure_raw != 0)
        {
            calculate(including_pressure);
        }
        else
        {
    ...
    float BMP180::temperature() const
    {
        return temperature_;
    }
    ...
    void BMP180::send_measure_command(domain::MeasureCommand command)
    {
    ...
    bool BMP180::read_measure_flag()
    {
        read_register(REGISTERS::MEASUREMENT_CTRL, 1);
        return ((data_buffer_[0] >> 5) & 0x01) == 0x00;
    }

    bool BMP180::measure(domain::MeasureCommand command, uint8_t attempts = 4u)
    {
        send_measure_command(command);
        bool measure_done = false;
        uint8_t attempt_count = 1u;
        while (measure_done == false)
        {
            sleep_ms(5u);
            measure_done = read_measure_flag();
    ...
    {
        bool temp_flag = measure(domain::MeasureCommand::Temperature);
        if (temp_flag == true)
        {
    ...
    {
        bool pres_flag = measure(domain::MeasureCommand::Pressure);
        if (pres_flag == true)
        {
    ...
    void BMP180::calculate(bool including_pressure)
    {
